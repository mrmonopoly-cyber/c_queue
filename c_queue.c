...
// static uint8_t !c_check_input_pointer(const void* ptr, const char* ptr_name){
//     if (!ptr) {
...
{
    if (!c_check_input_pointer(queue, "c_queue root pointer")) return EXIT_FAILURE;
    if (!c_check_input_pointer(ele, "c_queue element to push")) return EXIT_FAILURE;
    if (!ele_size){
    ...
    if (!(*queue)) {
        *queue = calloc(1, sizeof(**queue) + ele_size);
        if (!c_check_input_pointer(*queue, "c_queue new c_queue")) return EXIT_FAILURE;
...
{
    if (!c_check_input_pointer(queue, "c_queue root pointer")) return EXIT_FAILURE;
    if (!c_check_input_pointer(out, "out root pointer")) return EXIT_FAILURE;
    if (!c_check_input_pointer(*queue, "c_queue is empty")) return EXIT_FAILURE;

    if (!(*out)) {
        (*out) = calloc(1, (*queue)->ele_size);
        if (!c_check_input_pointer(*out, "c_queue new out buffer")) return EXIT_FAILURE;
    }
...
{
    if (!c_check_input_pointer(queue, "c_queue root pointer")) return ;
